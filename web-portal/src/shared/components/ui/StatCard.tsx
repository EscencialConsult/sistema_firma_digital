...
export function StatCard({ icon: Icon, label, value, detail, onClick }: { icon: LucideIcon; label: string; value: string; detail: string; onClick?: () => void }) {
  return (
    <div 
      className={`rounded-2xl border border-zinc-200/50 bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.01),0_10px_40px_-15px_rgba(0,0,0,0.04)] hover-lift ${onClick ? "cursor-pointer active:scale-[0.99] transition-all" : ""}`}
      onClick={onClick}
    >
